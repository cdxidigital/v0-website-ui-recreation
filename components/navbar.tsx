  ...
  return (
    <div className="w-full">
      ...
      <div className="bg-gradient-to-r from-pink-600 via-purple-600 to-pink-600 text-white text-center py-2 text-xs font-medium">
        <span role="status">Demo Platform: Experience our booking flow with sample data</span>
      </div>
      ...
      <header className="absolute top-[32px] left-0 right-0 z-50 bg-gradient-to-b from-slate-900/90 via-slate-900/60 to-transparent">
        <div className="container mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <Link
            href="/"
            className="flex items-center gap-2 transition-transform hover:scale-105 focus:scale-105"
            aria-label="WhoreDash home"
          >
            <Image
              src="/images/logo.png"
              alt="WhoreDash"
          ...
          <nav className="hidden md:flex items-center gap-8 text-sm font-medium" aria-label="Main navigation">
            {navLinks.map((item) => (
              <Link
                key={item.name}
                href={item.href}
                className="text-gray-300 hover:text-white transition-colors relative group py-2 focus:text-white"
              >
                {item.name}
                <span
                  className="absolute bottom-0 left-0 w-0 h-0.5 bg-current transition-all duration-300 group-hover:w-full group-focus:w-full"
                  aria-hidden="true"
                />
              </Link>
            ))}
          ...
          <div className="hidden md:flex items-center gap-4">
            ...
            <Link href="/pricing">
              <Button
                className={`${getButtonClass()} text-white rounded-full px-6 font-medium transition-all duration-300 btn-press shadow-lg hover:shadow-xl hover:scale-105`}
              >
                Join as Companion
              </Button>
            </Link>
        ...
        {isMobileMenuOpen && (
          <div
            id="mobile-menu"
            className="md:hidden bg-slate-900/95 backdrop-blur-lg border-t border-white/10 p-4 space-y-4 animate-fadeIn"
            role="dialog"
            aria-modal="true"
          >
